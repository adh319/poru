...
export class Player extends EventEmitter {
    ...
    public async play() {
        ...
        if (!this.currentTrack.track) this.currentTrack = await this.resolveTrack(this.currentTrack);
    ...
    private async resolveTrack(track: Track) {
        ...
        const result: any = await this.resolve({
            query,
    ...
    public moveNode(name: string) {
        ...
        if (!node.isConnected) throw new Error("Provided Node not is not connected");
    ...
    public async AutoMoveNode(): Promise<void> {
        if (this.poru.leastUsedNodes.length === 0) throw new Error("[Poru Error] No nodes are avaliable");
    ...
    async autoplay(requester) {
        try {
            let data = `https://www.youtube.com/watch?v=${this.previousTrack.info.identifier || this.currentTrack.info.identifier}&list=RD${
